...
class MultiGridEnv(minigrid.MiniGridEnv):
  ...
  def reset(self):
    ...
    try:
      ...
      print('Error resetting, check obs')
    ...
    for j in range(self.grid.height):
      for i in range(self.grid.width):
        ...
        if isinstance(cell, Agent):
          text += str(cell.agent_id) + agent_dir_to_str[self.agent_dir[cell.agent_id]]
          continue
        ...
        if cell.type == 'door':
          if cell.is_open:
            ...
            text += 'L' + cell.color[0].upper()
          else:
            text += 'D' + cell.color[0].upper()
          continue

        text += obj_to_str[cell.type] + cell.color[0].upper()
  ...
  def render(self, mode='human', close=False, highlight=True, tile_size=minigrid.TILE_PIXELS):
      ...
      if mode == 'human' and not self.window:
          from envs.gym_multigrid.window import Window
          self.window = Window('gym_minigrid')
          self.window.show(block=False)

      # Compute which cells are visible to the agent
      _, vis_mask = self.gen_obs_grid(0)
